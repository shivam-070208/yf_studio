...
import { FaLightbulb, FaToggleOn, FaBoxes, FaCogs,FaPlus } from 'react-icons/fa';
import Button from '../Button';

const services = [
  { title: 'Luminaires', icon: <FaLightbulb />, description: 'High-efficiency explosion-proof lighting solutions.' },
  { title: 'Control Station', icon: <FaToggleOn />, description: 'Reliable flameproof control and monitoring systems.' },
  { title: 'Junction Box', icon: <FaBoxes />, description: 'Flameproof enclosures for industrial wiring connections.' },
  { title: 'Instrumentation Panel', icon: <FaCogs />, description: 'Custom panels designed for precision monitoring and safety.' },
];
    ...
    <section className="relative py-20 px-4 md:px-16 bg-cover bg-center text-white bg-[url('https://html.themexriver.com/industo/images/main-slider/image-2.jpg')] bg-fixed ">
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
        {services.map((service, index) => (
          <div onMouseEnter={()=>sethovered(true)} onMouseLeave={()=>sethovered(false)} key={index} className="bg-[#1C0060] w-fit h-fit overflow-hidden -z-10 relative group text-white p-10 text-center shadow-md hover:shadow-xl transition-all">
            ...
            <p className="text-sm text-gray-300 mb-10 line-clamp-2 z-10">{service.description}</p>
